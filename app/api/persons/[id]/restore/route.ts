...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (existingPerson.status !== "DELETED") {
      return NextResponse.json({ error: "La personne n'est pas supprimée" }, { status: 400 })
    }
    ...
    return NextResponse.json({ error: "Erreur lors de la restauration de la personne" }, { status: 500 })
  }
}
