  ...
  const addDepartment = async (
    ...
    try {
      const response = await fetch("/api/departments", {
        ...
        body: JSON.stringify({ name, shortName, status: status.toUpperCase() }),
      });

      if (!response.ok) {
        const error = await response.json();
        throw new Error(
          error.error || "Erreur lors de la création du département"
        );
      }
  ...
  const updateDepartment = async (
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(
          error.error || "Erreur lors de la mise à jour du département"
        );
      }
  ...
  const restoreDepartment = async (id: string) => {
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(
          error.error || "Erreur lors de la restauration du département"
        );
      }
